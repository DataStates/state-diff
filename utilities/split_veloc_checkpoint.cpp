...
int main(int argc, char** argv) {
  ...
  if(file_size != expected_size) {
    std::cerr << "File size " << file_size << " does not match expected size " << expected_size << std::endl;
  }

  std::vector<std::string> names = {"NumParticles", "X", "Y", "Z", "VX", "VY", "VZ", "Potential", "GlobalIds", "Mask"};
  ...
  for(auto &e : region_info) {
    ...
    std::string outfile = std::string(argv[1]) + "." + names[e.first] + ".dat";
    std::ofstream outputfile(outfile, std::ofstream::out | std::ofstream::binary);
