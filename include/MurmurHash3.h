...
#ifndef __BASE_MURMUR3_HPP
#define __BASE_MURMUR3_HPP

namespace base_murmur3 {
#if defined(_MSC_VER)
...
inline void
MurmurHash3_x64_128(const void *key, uint64_t len, uint64_t seed[2],
                    void *out) {
    const uint8_t *data = (const uint8_t *) key;
...
inline void
MurmurHash3_x64_64(const void *key, uint64_t len, uint32_t seed, void *out) {
    const uint8_t *data = (const uint8_t *) key;
    const uint64_t nblocks = len / 16;
