...
class client_t {
    ...
    //                     Kokkos::View<uint64_t[1]> &num_changed,
    //                     Kokkos::View<uint64_t[1]> &num_comparisons);
    // void fill_work_queue(Queue &work_queue, uint32_t beg, uint32_t end);
...
void
fill_work_queue(Queue &work_queue, uint32_t beg, uint32_t end) {
    auto fill_policy = Kokkos::RangePolicy<>(beg, end);
    Kokkos::parallel_for(
        "Fill up queue with every node in the stop_level", fill_policy,
        KOKKOS_LAMBDA(const uint32_t i) { work_queue.push(i); });
    Kokkos::fence();   // Ensure completion of the parallel operation
}
...
std::cout << "Different hashes: " << ndifferent << std::endl;
    ...
    std::cout << "Number of different hashes after phase 1: "
              << diff_hash_vec.size() << std::endl;
    if (diff_hash_vec.size() > 0)
        compare_data_new_reader(prev, reader0, reader1, diff_hash_vec,
                                changed_chunks, num_changed, num_comparisons);
    // io_timer = io_reader.get_timer();
    //  prev.io_timer = prev.io_reader.get_timer();
    end = Timer::now();
    timers[1] = std::chrono::duration_cast<Duration>(end - beg).count();
...
client_t<DataType, Reader>::compare_trees(const client_t &prev, Queue &working_queue,
    ...
    // fill_work_queue(work_queue, level_beg, level_end + 1);
...
client_t<DataType, Reader>::compare_data(client_t &prev,
    ...
    if (num_diff_hash > 0) {
        ...
        for (size_t iter = 0; iter < num_iter; iter++) {
            ...
            Timer::time_point read_end = Timer::now();
            read_timer +=
                std::chrono::duration_cast<Duration>(read_end - read_beg)
                    .count();
            read_beg = Timer::now();
