...
void
gatherMismatchChunks_cpu(const float *data_a, const float *data_b,
                         std::vector<size_t> val_hash_vec, size_t num_first_occ,
                         int chunk_size, std::vector<float> &region_a_data,
    ...
    for (size_t i = 0; i < num_first_occ; i++) {
        ...
        std::copy(buffer_a.begin(), buffer_a.end(),
                  std::back_inserter(region_a_data));
        std::copy(buffer_b.begin(), buffer_b.end(),
                  std::back_inserter(region_b_data));
    }
}

void
gatherMismatchChunks_gpu(const float *data_a, const float *data_b,
                         Vector<size_t> val_hash_vec, size_t num_first_occ,
                         int chunk_size, Kokkos::View<uint8_t *> region_a_d,
                         Kokkos::View<uint8_t *> region_b_d) {
    ...
    Kokkos::View<uint8_t *, Kokkos::HostSpace,
        ...
        region_a_h(region_a_ptr, num_bytes);
    Kokkos::View<uint8_t *, Kokkos::HostSpace,
                 Kokkos::MemoryTraits<Kokkos::Unmanaged>>
        region_b_h(region_b_ptr, num_bytes);
...
void
hybridHash_cpu(size_t num_chunks, int chunk_size, float *data_a, float *data_b,
               float error, size_t &num_mismatch_one,
               size_t &num_mismatch_two) {
    ...
    for (size_t idx = 0; idx < num_chunks; idx++) {
        ...
        if (!same) {
            ...
            // Sanity check in case we missed any chunk that should have been a
            // mismatch
            int dsum = 0;
    ...
    if (num_mismatch > 0) {
        ...
        gatherMismatchChunks_cpu(data_a, data_b, mismatch_vec, num_mismatch,
                                 chunk_size, val_data_a, val_data_b);
...
void
hybridHash_gpu(size_t num_chunks, int chunk_size, Kokkos::View<float *> data_a,
               Kokkos::View<float *> data_b, float error,
               size_t &num_mismatch_one, size_t &num_mismatch_two) {
    Vector<size_t> val_hash_vec(num_chunks);

    // STAGE 1: Hash Generation
    Kokkos::parallel_reduce(
        "Hybrid::Hash", Kokkos::RangePolicy<>(0, num_chunks),
        KOKKOS_LAMBDA(size_t idx, size_t & lsum) {
            ...
            if (!same) {
                ...
                // Sanity check in case we missed any chunk that should have
                // been a mismatch
                int dsum = 0;
                for (int i = 0; i < num_element && dsum < 1; ++i) {
                    if (Kokkos::fabs(cur_data_a[i] - cur_data_b[i]) > error) {
    ...
    if (num_mismatch_one > 0) {
        ...
        Kokkos::View<float *>::HostMirror data_a_h =
            Kokkos::create_mirror_view(data_a);
        Kokkos::View<float *>::HostMirror data_b_h =
            Kokkos::create_mirror_view(data_b);
        ...
        gatherMismatchChunks_gpu(data_a_h.data(), data_b_h.data(), val_hash_vec,
                                 num_mismatch_one, chunk_size, val_data_a_d,
                                 val_data_b_d);

        // given the value of bitset at the index, change the digest or leave it
        // as is.
        Kokkos::parallel_reduce(
            "HybridMethod-Direct", Kokkos::RangePolicy<>(0, num_mismatch_one),
            KOKKOS_LAMBDA(size_t idx, size_t & lsum) {
                int num_element = chunk_size / sizeof(float);
                size_t offset = idx * num_element;
...
std::vector<float>
load_data_from_file(std::string filename, size_t start_offset) {
...
main(int argc, char **argv) {
    ...
    if (num_chunks * chunk_size < data_len * sizeof(float))
        num_chunks += 1;
    INFO("Params:: DataSize=" << data_len << ", ChunkSize=" << chunk_size
                              << ", NofChunks=" << num_chunks);
    ...
    Kokkos::initialize(argc, argv);
    {
        ...
        Kokkos::View<float *, Kokkos::HostSpace,
                     Kokkos::MemoryTraits<Kokkos::Unmanaged>>
            data_a_h((float *) data_a.data(), data_len);
        Kokkos::View<float *, Kokkos::HostSpace,
                     Kokkos::MemoryTraits<Kokkos::Unmanaged>>
            data_b_h((float *) data_b.data(), data_len);
        ...
        for (int i = 0; i < num_tests; i++) {
            ...
            for (size_t idx = 0; idx < num_chunks; idx++) {
                int num_element = chunk_size / sizeof(float);
                size_t offset = idx * num_element;
                direct_mismatch += areAbsoluteEqual(data_a.data() + offset,
                                                    data_b.data() + offset,
                                                    num_element, error, idx);
            }
            ...
            */
            ...
            if (direct_mismatch == hybrid_mismatch_gpu_two) {
                INFO("Test"
                     << i << " (e=" << error << "): Dir=" << direct_mismatch
                     << "; Ours=" << hybrid_mismatch_gpu_two << ".....OK");
            } else {
                INFO("Test"
                     << i << " (e=" << error << "): Dir=" << direct_mismatch
                     << "; Ours=" << hybrid_mismatch_gpu_two << ".....FAIL");
                fail_count += 1;
            }
        }
        INFO("Validation result: " << fail_count << "/" << num_tests
                                   << " tests failed");
    }
    Kokkos::finalize();
    return 0;
