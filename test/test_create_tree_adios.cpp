...
void adios_writer(adios2::ADIOS &adios, std::vector<uint8_t> &buffer) {
  adios2::IO io = adios.DeclareIO("tree-writer");
  io.SetEngine("BP5");
  adios2::Variable<uint8_t> adios_tree = 
    io.DefineVariable<uint8_t>("tree0", {buffer.size()}, {0}, {buffer.size()});
  adios2::Engine writer = io.Open("run0_tree.bp", adios2::Mode::Write);
  writer.Put<uint8_t>(adios_tree, buffer.data());
  writer.Close();
}

std::vector<uint8_t> adios_reader(adios2::ADIOS &adios) {
...
int main(int argc, char** argv) {
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    std::cout << "EXEC STATE:: Tree serialized to ADIOS2's BP file" << std::endl;
    ...
    std::cout << "EXEC STATE:: Tree Read from ADIOS2's BP file and deserialized" << std::endl;
