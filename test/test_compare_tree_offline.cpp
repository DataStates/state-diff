...
bool
write_file(std::string filename, uint8_t *buffer, size_t size) {
    ...
    while (remaining > 0) {
        ...
        if (ret == -1)
            FATAL("cannot write " << size << " bytes to " << filename
...
main(int argc, char **argv) {
    ...
    size_t stream_buffer_len =
        512 * 1024 * 1024 /
        sizeof(float);   // how much data element to read in during the second
...
#pragma omp for
        ...
        Kokkos::View<uint8_t *> data_run0_d("Run0", data_size),
            data_run1_d("Run1", data_size), data_run2_d("Run2", data_size);
        using DataHostView =
        ...
        tree_object.create_tree((uint8_t *) data_run2_d.data(),
                                data_run2_d.size());
        ...
        CompareTreeDeduplicator comparator_01(
            chunk_size, root_level, fuzzy_hash, error_tolerance, dtype[0]);
        comparator_01.setup(data_size, stream_buffer_len / sizeof(float),
                            run0_file, run1_file);
        comparator_01.deserialize(tree0_data,
                                  tree1_data);   // merkle trees of each run
        comparator_01.compare_trees_phase1();    // compare trees
        if (comparator_01.diff_hash_vec.size() > 0)
            comparator_01.compare_trees_phase2();   // compare data for
                                                    // mismatched data chunks
        std::cout << "Number of mismatch (0-1) = "
                  << comparator_01.get_num_changes() << std::endl;

        // Compare checkpoints 0 and 2. These two checkpoints should NOT match
        CompareTreeDeduplicator comparator_02(
            chunk_size, root_level, fuzzy_hash, error_tolerance, dtype[0]);
        comparator_02.setup(data_size, stream_buffer_len / sizeof(float),
                            run0_file, run2_file);
        comparator_02.deserialize(tree0_data,
                                  tree2_data);   // merkle trees of each run
        comparator_02.compare_trees_phase1();    // compare trees
        if (comparator_02.diff_hash_vec.size() > 0)
            comparator_02.compare_trees_phase2();   // compare data for
                                                    // mismatched data chunks
        std::cout << "Number of mismatch (0-2) = "
                  << comparator_02.get_num_changes() << std::endl;

        if (comparator_01.get_num_changes() != 0 ||
            comparator_02.get_num_changes() != data_len)
            test_status = -1;
    }
    Kokkos::finalize();
