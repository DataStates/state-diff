...
template <typename T>
std::vector<T>
adios_reader(adios2::IO &io, std::string tag, std::string fn) {
    io.SetEngine("BP5");
...
#pragma omp for
        ...
        std::vector<uint8_t> deserialized_tree1 =
            adios_reader<uint8_t>(io_reader, "tree1", "run1_tree.bp");
        std::cout << "Successfully deserialized" << std::endl;
        CompareTreeDeduplicator comparator(chunk_size, root_level, fuzzy_hash,
                                           error_tolerance, dtype[0]);
        comparator.setup(data_size, stream_buffer_len / sizeof(float),
                         run0_file, run1_file);
        comparator.deserialize(
            deserialized_tree0,
            deserialized_tree1);             // merkle trees of each run
        comparator.compare_trees_phase1();   // compare trees
        if (comparator.diff_hash_vec.size() > 0)
            comparator.compare_trees_phase2();   // compare data for mismatched
                                                 // data chunks
        std::cout << "Number of Different Values = "
                  << comparator.get_num_changes() << std::endl;
        std::cout << "EXEC STATE:: Tree Read from ADIOS2's BP file and compared"
                  << std::endl;
