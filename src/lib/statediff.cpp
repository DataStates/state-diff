...
client_t<DataType>::client_t(int id, io_reader_t<DataType> &reader,
...
template <typename DataType>
void
client_t<DataType>::create(const std::vector<DataType> &data) {
    ...
    Kokkos::parallel_for(
        ...
        Kokkos::RangePolicy<>(0, num_chunks), KOKKOS_LAMBDA(uint32_t idx) {
            ...
            // Hash chunk
            if (use_fuzzy_hash) {
                tree_curr.calc_leaf_fuzzy_hash(data_ptr + offset, num_bytes,
                                               err_tol, dtype, leaf);
            } else {
...
template <typename DataType>
size_t
client_t<DataType>::deserialize(std::vector<uint8_t> &buffer) {
    size_t offset = 0;
...
template <typename DataType>
bool
client_t<DataType>::compare_with(const client_t &prev) {
    ASSERT(tree != nullptr && prev.tree != nullptr);
    ASSERT(num_chunks == prev.num_chunk);
            ...
            KOKKOS_LAMBDA(uint32_t i) {
                ...
                if (!identical) {
                    if ((n_chunks - 1 <= node) && (node < n_nodes)) {
                        if (node <
                            left_leaf) {   // Leaf is not on the last level
...
template <typename DataType>
size_t
client_t<DataType>::compare_data(const client_t &prev,
                                 Vector<size_t> &diff_hash_vec,
    ...
    if (num_diff_hash > 0) {
        ...
        if (num_iter * io_reader.chunks_per_slice < num_diff_hash)
            num_iter += 1;
...
template <typename DataType>
double
client_t<DataType>::get_io_time() const {
    return io_timer[0];
}
