...
class MMapStream {
 ...
 public:
  ...
  size_t get_num_slices() const {
    size_t num_slices = num_offsets / chunks_per_slice;
    if(num_slices * chunks_per_slice < num_offsets) 
      num_slices += 1;
    return num_slices;
  }
  ...
  size_t prepare_slice() {
    if(full_transfer) {
      ...
      int active_threads = static_cast<size_t>(num_threads) > chunks_left ? chunks_left : num_threads;
